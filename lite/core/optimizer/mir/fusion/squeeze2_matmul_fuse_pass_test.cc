...
// squeeze2([2,3,1,1], axes=[2,3]) → matmul → mul
std::vector<TestOpDesc> MakeSqueeze2Matmul() {
  std::vector<TestOpDesc> ops;
  ops.push_back({"squeeze2",
                 {{"X", {"x"}}},
                 {{"Out", {"sq_out"}}, {"XShape", {"xs"}}},
                 {},
                 {},
                 {},
                 {},
...
TEST(Squeeze2MatmulFuser, fuse_to_mul) {
  auto scope = std::make_shared<Scope>();
  auto* x = scope->Var("x")->GetMutable<lite::Tensor>();
  x->Resize(DDim({2, 3, 1, 1}));  // rank 4, axes [2,3]
  auto* xs = scope->Var("xs")->GetMutable<lite::Tensor>();
  xs->Resize(DDim({5}));
  auto* sq = scope->Var("sq_out")->GetMutable<lite::Tensor>();
  sq->Resize(DDim({2, 3}));
  ...
  auto graph = BuildGraph(MakeSqueeze2Matmul(), {}, scope.get());
...
TEST(Squeeze2MatmulFuser, skip_wrong_axes) {
  auto scope = std::make_shared<Scope>();
  auto* x = scope->Var("x")->GetMutable<lite::Tensor>();
  ...
  std::vector<TestOpDesc> ops = MakeSqueeze2Matmul();
  ops[0].int_vector_attrs = {{"axes", {0, 1}}};  // wrong axes
